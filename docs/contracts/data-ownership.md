...
| Business Payment / customer-facing Stripe payment state | Growth Engine | Other apps by reference/snapshot only when necessary |
...
SaaS Subscription data is separate from Growth Engine Business Payment data.

Numeria Studio owns the Numeria Studio Pro SaaS subscription state for the Free / Pro release. Velvet owns Velvet Pro SaaS subscription state when Velvet Pro billing is released.

Allowed SaaS subscription records/projections:
...
- webhook processing status and timestamps.

SaaS subscription records must not contain card data, Stripe secrets, webhook secrets, raw provider payloads, invoice bodies, or Growth Engine Payment/Sales ledger entries.

Growth Engine remains canonical for customer-facing Business Payment, Payment, Sales, Reservation, Customer, Public Site, and Business workflow records.
...
Growth Engine is the canonical owner of Business Payment, Payment, Sales and Revenue.
...
Plan support does not change Source of Truth ownership.

Rules:

- Plan records and entitlement decisions must reference `workspaceId + userId / ownerUserId`.
- `professionalId` is not required for MVP plan enforcement.
- Numeria appraisal subjects are Numeria-owned snapshots for appraisal workflows, not canonical Customer master data.
- Velvet customer-scoped professional memory belongs to Velvet, but Customer master, Reservation, Payment, and Sales remain Growth Engine-owned.
- AI Platform Core may own AI Usage and AI Activity generated by plan-gated AI capabilities.
- Feedback Hub may store plan metadata attached to feedback, but does not own subscription, billing, or payment truth.
- Pricing values must not drive entitlement identity; plan behavior must key from `productCode`, `planId`, `SubscriptionStatus`, `Entitlement`, and `FeatureKey`.

## Free Pro Business Data Ownership

Plan support does not change Source of Truth ownership.

Numeria Studio must not duplicate Customer master, Reservation, Payment, Sales, Conversation, Message, AI Activity, or AI Usage.
...
AI Platform Core may store AI usage metadata and activity records, but not full appraisal, consultation, conversation, message, customer master, payment, secret, or secret prompt payloads.

Feedback Hub may store inquiry metadata and user-reported descriptions, but it must not store API keys, Secrets, Stripe Secrets, card information, payment details, full appraisal text, full conversation text, or full customer master records.
